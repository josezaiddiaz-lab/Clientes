...
# --- EDITAR O ELIMINAR REGISTROS (ACTUALIZADO DINÁMICAMENTE) ---
st.markdown("---")
with st.expander("✏️ Editar o Eliminar Registros"):
    if df.empty:
        ...
        if seleccion == "-- Selecciona un registro --":
            ...
            # Detectar cambio de selección para refrescar los inputs al instante sin recargar la página
            if "last_edit_selection" not in st.session_state or st.session_state.last_edit_selection != seleccion:
                ...
                st.session_state.e_iva = str(fila_df.get("IVA", ""))
                st.session_state.e_isr = str(fila_df.get("ISR", ""))
            ...
            with st.form("form_editar_factura"):
                ...
                calc_e_iva = f"{e_subtotal * 0.16:.2f}" if str(st.session_state.get("e_iva", "")) == "" else st.session_state.get("e_iva", "")
                calc_e_isr = f"{e_subtotal * 0.0125:.2f}" if str(st.session_state.get("e_isr", "")) == "" else st.session_state.get("e_isr", "")
                
                e_iva_input = st.text_input("IVA", value=calc_e_iva, key="e_iva_input")
                e_isr_input = st.text_input("ISR", value=calc_e_isr, key="e_isr_input")
