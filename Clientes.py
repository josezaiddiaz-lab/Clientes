...
def recalcular_y_sincronizar_tarjeta_tc():
    """Función de apoyo para forzar la actualización de la hoja 'Abono a tarjeta de crédito' cuando cambian los Clientes."""
    try:
    ...
    with st.sidebar.form(f"alta_form_{pestana_seleccionada}", clear_on_submit=True):
        ...
        if submitted:
            if pestana_seleccionada == "Abono a tarjeta de crédito":
                ...
                valido = bool(desc_upper) and abono_tc >= 0.0
                ...
                if valido:
                    try:
                        ...
                        t_debito_val = calcular_total_tarjeta_debito_pagada()
                        saldo_tc_calculado = max(0.0, t_debito_val - abono_tc)
                        ...
                        new_row = {
                            ...
                            "Saldo TC": f"{saldo_tc_calculado:.2f}",
                            "FECHA": str(fecha)
                        }
                        ...
                        st.error(f"Error al conectar con Google Sheets: {e_error}")
                else:
                    st.error("Por favor completa la descripción y un abono válido.")
            else:
                ...
                if valido and subtotal >= 0:
                    facturas_existentes = [str(row.get("FACTURA", "")).strip() for _, row in df.iterrows() if str(row.get("FACTURA", "")).strip() and not str(row.get("FACTURA", "")).startswith("---") and not str(row.get("FACTURA", "")).startswith("TOTALES") and not str(row.get("FACTURA", "")).startswith("El mes")]
    ...
    with st.expander(f"✏️ Editar o Eliminar en {pestana_seleccionada}"):
        if df.empty:
            ...
            for idx, row in df.iterrows():
                if pestana_seleccionada == "Abono a tarjeta de crédito":
                    val_id = str(row.get('Abono TC', row.get('ABONO TC', ''))).strip()
                    desc_id = str(row.get('Descripcion', row.get('DESCRIPCION', ''))).strip()
                    if desc_id and not desc_id.startswith("---") and not desc_id.startswith("TOTALES") and not desc_id.startswith("El mes"):
