    ...
    for _, row in df_actual.iterrows():
        ...
        if identificador and not identificador.startswith("---") and not identificador.startswith("TOTALES"):
            ...
            if tipo_seccion == "Abono a tarjeta de crédito":
                for col_num in ["ABONO TC", "SALDO TC"]:
    ...
    for reg in registros:
        ...
        if dt:
            if dt.month != mes_actual:
                ...
                if tipo_seccion == "Abono a tarjeta de crédito":
                    utilidad_m = calcular_utilidad_mes(mes_actual)
                    if len(df_actual.columns) > 1:
                        fila_div[df_actual.columns[1]] = f"Utilidad al iniciar el mes: {utilidad_m:.2f}"
...
if pestana_seleccionada == "Utilidad":
    ...
    # Recálculo automático de sumas y saldos para tarjeta de crédito si aplica
    if pestana_seleccionada == "Abono a tarjeta de crédito" and not df.empty:
        # Calcular dinámicamente los totales de abonos pagados por mes y actualizar tarjeta de débito y saldo TC si es necesario
        pass
    ...
    with st.sidebar.form(f"alta_form_{pestana_seleccionada}", clear_on_submit=True):
        ...
        if submitted:
            if pestana_seleccionada == "Abono a tarjeta de crédito":
                ...
                if valido:
                    # Calcular suma de abonos pagados del mes o tarjeta debito según lo requerido
                    # Aquí sumamos los abonos que correspondan al mes de la fecha ingresada
                    current_records = []
                    if not df.empty:
                        for _, r in df.iterrows():
                    ...
                    # Simular la inserción temporal para calcular la suma total de abonos del mes
                    temp_nueva_lista = current_records + [{
                    ...
                    # Sumar todos los Abono TC del mes correspondiente
                    suma_mes_abonos = sum(limpiar_numero(r.get("ABONO TC", 0)) for r in temp_nueva_lista if parsear_fecha(r.get("FECHA", "")) and parsear_fecha(r.get("FECHA", "")).month == fecha.month)
                    
                    new_row = {
                        "Mes": meses_nombres.get(fecha.month, "--- MES ---"),
                        "Abono TC": f"{abono_tc:.2f}",
                        "Descripcion": desc_upper,
                        "Tarjeta Debito": f"{suma_mes_abonos:.2f}",
            ...
            if seleccion == "-- Selecciona un registro --":
                ...
                with st.form("form_editar_factura"):
                    if pestana_seleccionada == "Abono a tarjeta de crédito":
                        ...
                        if actualizar:
                            dt_edit = parsear_fecha(e_fecha)
                            mes_num_edit = dt_edit.month if dt_edit else 1
                            ...
                            # Recalcular tarjeta débito (suma de abonos del mes) y saldo TC
                            suma_mes_abonos = sum(limpiar_numero(r.get("ABONO TC", 0)) for r in current_records if parsear_fecha(r.get("FECHA", "")) and parsear_fecha(r.get("FECHA", "")).month == mes_num_edit)
                            
                            for r in current_records:
                                dt_r = parsear_fecha(r.get("FECHA", ""))
                                if dt_r and dt_r.month == mes_num_edit:
                                    r["Tarjeta Debito"] = f"{suma_mes_abonos:.2f}"
                                if parsear_fecha(r.get("FECHA", "")) == dt_edit and limpiar_numero(r.get("ABONO TC", 0)) == e_abono:
                                    r["Saldo TC"] = f"{e_abono:.2f}"
                                r["Mes"] = meses_nombres.get(dt_r.month if dt_r else 1, "--- MES ---")
