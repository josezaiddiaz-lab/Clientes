...
# Función callback para actualizar automáticamente IVA e ISR en el alta si cambian el subtotal
def actualizar_calculos_alta():
    sub = st.session_state.alta_sub
    st.session_state.alta_iva = f"{sub * 0.16:.2f}"
    st.session_state.alta_isr = f"{sub * 0.0125:.2f}"
...
with st.sidebar.form("alta_form"):
    ...
    subtotal = st.number_input(
        "SUBTOTAL", 
        min_value=0.0, 
        value=st.session_state.val_subtotal, 
        format="%.2f", 
        key="alta_sub",
        on_change=actualizar_calculos_alta
    )
    
    # Inicializar valores por defecto si están vacíos
    if "alta_iva" not in st.session_state:
        st.session_state.alta_iva = f"{subtotal * 0.16:.2f}"
    if "alta_isr" not in st.session_state:
        st.session_state.alta_isr = f"{subtotal * 0.0125:.2f}"

    iva_input = st.text_input("IVA (Opcional, por defecto 16%)", value=st.session_state.alta_iva, key="alta_iva_input")
    isr_input = st.text_input("ISR (Opcional, por defecto 1.25%)", value=st.session_state.alta_isr, key="alta_isr_input")
    ...
    if submitted:
        if factura and cliente and descripcion and fecha and subtotal >= 0:
            ...
            if factura.strip() in facturas_existentes:
                ...
                st.session_state.val_fecha = datetime.today()
                st.session_state.val_subtotal = 0.0
                st.session_state.alta_iva = ""
                st.session_state.alta_isr = ""
...
with st.expander("✏️ Editar o Eliminar Registros"):
    if df.empty:
        ...
        if seleccion == "-- Selecciona un registro --":
            ...
            # Buscamos directamente la fila usando Pandas y el índice real para evitar desfases
            fila_df = df.iloc[idx_seleccionado - 2]
            
            e_factura_val = str(fila_df.get("FACTURA", ""))
            e_cliente_val = str(fila_df.get("CLIENTES", ""))
            e_descripcion_val = str(fila_df.get("DESCRIPCION", ""))
            e_fecha_val = str(fila_df.get("FECHA", ""))
            
            try:
                sub_val = float(fila_df.get("SUBTOTAL", 0))
            except ValueError:
                sub_val = 0.0
                
            e_iva_val = str(fila_df.get("IVA", ""))
            e_isr_val = str(fila_df.get("ISR", ""))
            e_estatus_val = str(fila_df.get("ESTATUS", "PENDIENTE"))

            # Callback para recalcular edición en tiempo real al mover el subtotal
            def actualizar_calculos_edit():
                sub_edit = st.session_state.e_sub
                st.session_state.e_iva = f"{sub_edit * 0.16:.2f}"
                st.session_state.e_isr = f"{sub_edit * 0.0125:.2f}"

            # Mantener sincronizado si carga por primera vez el registro seleccionado
            if "e_sub" not in st.session_state or st.session_state.get("last_selected") != seleccion:
                st.session_state.e_sub = sub_val
                st.session_state.e_iva = e_iva_val
                st.session_state.e_isr = e_isr_val
                st.session_state.last_selected = seleccion

            e_factura = st.text_input("Factura", value=e_factura_val, key="e_fact")
            e_cliente = st.text_input("Clientes", value=e_cliente_val, key="e_cli")
            e_descripcion = st.text_input("Descripción", value=e_descripcion_val, key="e_desc")
            e_fecha = st.text_input("Fecha (YYYY-MM-DD)", value=e_fecha_val, key="e_fec")
            
            e_subtotal = st.number_input("Subtotal", value=st.session_state.e_sub, format="%.2f", key="e_sub", on_change=actualizar_calculos_edit)
            e_iva_input = st.text_input("IVA", value=st.session_state.e_iva, key="e_iva")
            e_isr_input = st.text_input("ISR", value=st.session_state.e_isr, key="e_isr")
            
            idx_estatus = 0 if e_estatus_val != "PAGADA" else 1
            e_estatus = st.selectbox("Estatus", options=["PENDIENTE", "PAGADA"], index=idx_estatus, key="e_est")
            
            col1, col2 = st.columns(2)
            actualizar = col1.button("Actualizar Registro")
            eliminar = col2.button("Eliminar Registro")
