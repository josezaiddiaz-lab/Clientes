...
# --- CONFIGURACION DE GOOGLE SHEETS ---[cite: 1]
scope = [
...
def calcular_total_tarjeta_debito_pagada():
    ...
    for _, row in df_c.iterrows():
        ...
        if fact and not fact.startswith("---") and not fact.startswith("TOTALES") and not fact.startswith("El mes"):
            if estatus == "PAGADA" and fact not in seen_facturas:
                seen_facturas.add(fact)
    ...
    try:
        ...
        if not df_tc.empty:
            ...
            for _, r in df_tc.iterrows():
                ...
                fecha_check = str(r.get("FECHA", "")).strip()
                abono_check = str(r.get("Abono TC", r.get("ABONO TC", ""))).strip()
                key_tc = f"{fecha_check}-{desc_check}-{abono_check}"
...
with col_metrica:
    ...
    if not df_temp_tc.empty:
        ...
        for _, r in df_temp_tc.iterrows():
            ...
            if desc_chk and not desc_chk.startswith("---") and not desc_chk.startswith("TOTALES") and not desc_chk.startswith("El mes") and desc_chk.upper() != "PRUEBA":
                dt = parsear_fecha(r.get("FECHA", ""))
                registros_validos.append((dt if dt else datetime.min, limpiar_numero(s_val)))
        
        if registros_validos:
            registros_validos.sort(key=lambda x: x[0])
            ultimo_saldo_tc = registros_validos[-1][1]
        else:
...
if pestana_seleccionada == "Utilidad":
    ...
    if pestana_seleccionada == "Abono a tarjeta de crédito" and df.empty:
        try:
            sheet.clear()
            sheet.update([["Mes", "Abono TC", "Descripcion", "Tarjeta Debito", "Saldo TC", "FECHA"]])
            df = load_data(pestana_seleccionada)
        except Exception:
            pass
            ...
            with st.form("form_editar_factura"):
                ...
                if eliminar:
                    ...
                    for i, r in df.iterrows():
                        ...
                        desc_chk = str(r.get("Descripcion", r.get("DESCRIPCION", ""))).strip()
                        if val_check and not val_check.startswith("---") and not val_check.startswith("TOTALES") and not val_check.startswith("El mes") and desc_chk.upper() != "PRUEBA":
                            current_records.append(r.to_dict())
