...
def reorganizar_con_meses_en_sheet(df_actual, tipo_seccion):
    ...
    for _, row in df_actual.iterrows():
        # Para la nueva sección usamos ABONO TC o FACTURA como identificador principal
...
pestana_seleccionada = st.radio(
    ...
    options=["Clientes", "Proveedores", "Gastos", "Abono a tarjeta de crédito", "Utilidad"],
    horizontal=True
)
...
if pestana_seleccionada == "Utilidad":
    ...
    with st.sidebar.form(f"alta_form_{pestana_seleccionada}", clear_on_submit=True):
        if pestana_seleccionada == "Abono a tarjeta de crédito":
            ...
            tarjeta_debito = st.text_input("TARJETA DEBITO", value="")
            saldo_tc = st.number_input("SALDO TC", min_value=0.0, value=0.0, format="%.2f")
        else:
        ...
        if submitted:
            if pestana_seleccionada == "Abono a tarjeta de crédito":
                ...
                td_upper = tarjeta_debito.strip().upper()
                valido = bool(desc_upper and td_upper)
                ...
                if valido:
                    new_row = {
                        ...
                        "TARJETA DEBITO": td_upper,
                        "SALDO TC": f"{saldo_tc:.2f}",
                        "FECHA": str(fecha)
                    }
            ...
            if seleccion == "-- Selecciona un registro --":
                ...
                if "last_edit_selection" not in st.session_state or st.session_state.last_edit_selection != seleccion:
                    ...
                    if pestana_seleccionada == "Abono a tarjeta de crédito":
                        ...
                        st.session_state.e_td = str(fila_df.get("TARJETA DEBITO", ""))
                        try:
                            st.session_state.e_saldo = float(str(fila_df.get("SALDO TC", 0)).replace(',', '.'))
                        except ValueError:
                            st.session_state.e_saldo = 0.0
                ...
                with st.form("form_editar_factura"):
                    if pestana_seleccionada == "Abono a tarjeta de crédito":
                        ...
                        e_tarjeta = st.text_input("Tarjeta Débito", key="e_td")
                        e_saldo = st.number_input("Saldo TC", format="%.2f", key="e_saldo")
                        ...
                        if actualizar:
                            updated_row = {
                                "MES": meses_nombres.get(parsear_fecha(e_fecha).month if parsear_fecha(e_fecha) else 1, "--- MES ---"),
                                "ABONO TC": f"{e_abono:.2f}",
                                "DESCRIPCION": e_descripcion.strip().upper(),
                                "TARJETA DEBITO": e_tarjeta.strip().upper(),
                                "SALDO TC": f"{e_saldo:.2f}",
                                "FECHA": e_fecha.strip()
                            }
