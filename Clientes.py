...
with st.sidebar.form("alta_form"):
    ...
    # Cálculo automático dinámico basado en el subtotal actual del formulario
...
with st.expander("✏️ Editar o Eliminar Registros"):
    if df.empty:
        ...
        if seleccion == "-- Selecciona un registro --":
            ...
            # Sincronizar al cambiar de registro seleccionado en la edición
            ...
            e_subtotal = st.number_input("Subtotal", value=st.session_state.e_sub, format="%.2f", key="e_sub")
            
            # Cálculo automático en tiempo real para edición (fuera de formularios ya que la sección de edición no usa st.form)
            calc_e_iva = f"{e_subtotal * 0.16:.2f}" if e_iva_val == "" else e_iva_val
            calc_e_isr = f"{e_subtotal * 0.0125:.2f}" if e_isr_val == "" else e_isr_val
            
            e_iva_input = st.text_input("IVA", value=calc_e_iva, key="e_iva")
            e_isr_input = st.text_input("ISR", value=calc_e_isr, key="e_isr")
