...
def limpiar_numero(val):
    """Limpia cadenas de texto para convertirlas a float, manejando diversos formatos."""
    if val is None:
        return 0.0
...
def calcular_tabla_utilidad():
    """Genera automáticamente la estructura de la pestaña Utilidad basada en los meses de Clientes, Proveedores y Gastos."""
    ...
    def extraer_subtotales_por_mes(df):
        ...
        for _, row in df.iterrows():
            ...
            # Si es una fila de registro normal, acumular el subtotal
            if fact and not fact.startswith("---") and not fact.startswith("TOTALES"):
    ...
    # Obtener todos los meses únicos presentes en cualquiera de las 3 secciones
    meses_unidos = sorted(list(set(list(sub_clientes.keys()) + list(sub_proveedores.keys()) + list(sub_gastos.keys()))))
    
    filas_utilidad = []
    for m in meses_unidos:
        # Añadir separador del mes
        filas_utilidad.append({
            "Subtotal de Clientes Del mes": meses_nombres.get(m, "--- MES ---"),
...
# Selector principal de vistas arriba (Incluyendo Utilidad)
pestana_seleccionada = st.radio(
    "Selecciona la sección a gestionar:",
    options=["Clientes", "Proveedores", "Gastos", "Utilidad"],
    horizontal=True
)
...
if pestana_seleccionada == "Utilidad":
    ...
    # Sincronizar automáticamente la pestaña de Utilidad en Google Sheets
    try:
        sheet_utilidad = spreadsheet.worksheet("Utilidad")
        headers_util = ["Subtotal de Clientes Del mes", "Subtotal de Proveedores Del mes", "Subtotal de Gastos Del mes", "Total"]
        ...
        pass # Si la pestaña no existe aún en Google Sheets, se puede crear manualmente o se omite el error de sync automático.
