...
if pestana_seleccionada == "Utilidad":
    ...
    # Auto-corrección al cargar: normaliza decimales largos a 2 decimales en la hoja y recalcula totales si es Clientes
    if not df.empty:
        registros_a_normalizar = []
        cambio_necesario = False
        for _, r in df.iterrows():
            r_dict = r.to_dict()
            fact = str(r_dict.get("FACTURA", "")).strip()
            if fact and not fact.startswith("---") and not fact.startswith("TOTALES"):
                for col_num in ["SUBTOTAL", "IVA", "ISR", "TOTAL"]:
                    if col_num in r_dict and str(r_dict[col_num]).strip() != "":
                ...
                # Asegurar fórmula correcta en Clientes (Subtotal + IVA - ISR)
                ...
                if f_upper in facturas_existentes:
                    ...
                    if pestana_seleccionada == "Clientes":
                        try:
                            ...
                            isr = 0.0
                        total = subtotal + iva - isr  # <--- CAMBIO APLICADO: RESTA DE ISR
            ...
            if seleccion == "-- Selecciona un registro --":
                ...
                with st.form("form_editar_factura"):
                    ...
                    if pestana_seleccionada == "Clientes":
                        ...
                        calc_e_tot = e_subtotal + calc_e_iva - calc_e_isr  # <--- RESTA EN VISTA PREVIA EDICIÓN
                        ...
                        if e_fact_upper in facturas_existentes:
                            ...
                            if pestana_seleccionada == "Clientes":
                                try:
                                    ...
                                    e_isr = 0.0
                                e_total = e_subtotal + e_iva - e_isr  # <--- RESTA AL ACTUALIZAR
