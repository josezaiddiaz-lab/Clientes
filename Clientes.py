...
# Conectamos con el archivo principal de Google Sheets
spreadsheet = client.open('Clientes y Proveedores')
...
if "val_col2" not in st.session_state: st.session_state.val_col2 = "" # CLIENTE o RFC
if "val_col3" not in st.session_state: st.session_state.val_col3 = "" # CLIENTES o PROVEEDOR
...
with st.sidebar.form(f"alta_form_{pestana_seleccionada}", clear_on_submit=True):
    ...
    else: # Proveedores y Gastos (tienen FACTURA, RFC, PROVEEDOR/DESCRIPCION, etc.)
        # Según tu captura de proveedores/gastos: FACTURA, RFC, PROVEEDOR, DESCRIPCION, FECHA, SUBTOTAL, IVA, TOTAL, ESTATUS
        col2_label = "RFC"
