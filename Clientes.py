...
# Selector ubicado en la barra lateral izquierda
...
if pestana_seleccionada == "Utilidad":
    ...
    with st.form("form_agregar_registro", clear_on_submit=False):
        if pestana_seleccionada == "Abono a tarjeta de crédito":
            ...
            if submit_agregar:
                ...
                if not f_desc_upper or not f_fecha.strip():
                    st.error("Por favor completa la descripción y la fecha.")
                else:
                    nuevo_reg = {
                        "Mes": "",
                    ...
                    st.success("¡Registro guardado y sincronizado con éxito!")
                    st.rerun()
        else:
            ...
            if pestana_seleccionada == "Clientes":
                ...
                st.info(f"Total estimado: ${calc_tot:.2f}")
            ...
            if submit_agregar:
                ...
                if not f_fact_upper or not f_fecha.strip():
                    st.error("Por favor completa al menos la Factura y la Fecha.")
                else:
                    facturas_existentes = []
                    if not df.empty:
                        for _, r in df.iterrows():
                    ...
                    if f_fact_upper in facturas_existentes:
                        st.error(f"¡Error! La factura '{f_fact_upper}' ya existe en esta sección.")
                    else:
                        try:
    ...
    if not df.empty:
        for i, r in df.iterrows():
            ...
            if identif and not identif.startswith("---") and not identif.startswith("TOTALES") and not identif.startswith("El mes"):
                ...
                opciones_registros.append(f"Fila {i+2} - {identif} ({fec})")
                indices_reales.append(i + 2)
                
    if len(opciones_registros) <= 1:
        ...
        def actualizar_campos_edicion():
            ...
            if seleccion_actual != "-- Selecciona un registro --" and seleccion_actual in opciones_registros:
                idx_sel = indices_reales[opciones_registros.index(seleccion_actual)]
