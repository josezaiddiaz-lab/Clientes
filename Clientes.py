...
def reorganizar_con_meses_en_sheet(df_actual, tipo_seccion):
    ...
    # Agrupar por mes incluso si no hay abonos registrados, basándose en la fecha actual o la de los registros
    if not registros and tipo_seccion == "Abono a tarjeta de crédito":
        ...
        fila_fin[df_actual.columns[0]] = f"El mes termino con = {t_debito_actual:.2f}"
        nueva_data.append(fila_fin)
        
        fila_tot = {col: "" for col in df_actual.columns}
        if "Mes" in df_actual.columns: fila_tot["Mes"] = "TOTALES DEL MES"
        elif "Abono TC" in df_actual.columns: fila_tot["Abono TC"] = "TOTALES DEL MES"
        if "Abono TC" in df_actual.columns: fila_tot["Abono TC"] = "0.00"
        if "Saldo TC" in df_actual.columns: fila_tot["Saldo TC"] = f"{t_debito_actual:.2f}"
        nueva_data.append(fila_tot)
        return nueva_data
    ...
    try:
        ...
        if not df_tc.empty:
            for _, r in df_tc.iterrows():
                ...
                if desc_check and not desc_check.startswith("---") and not desc_check.startswith("TOTALES") and not desc_check.startswith("El mes"):
...
with col_metrica:
    # Sincronizar automáticamente la tarjeta al cargar la página si hay pagos nuevos
    ...
    if not df_temp_tc.empty:
        for _, r in df_temp_tc.iterrows():
            ...
            if s_val and not s_val.startswith("---") and not s_val.startswith("TOTALES") and not s_val.startswith("El mes"):
                ultimo_saldo_tc = limpiar_numero(s_val)
    
    if ultimo_saldo_tc == 0.0:
        # Si no hay saldo en las filas, tomar directamente el total pagado de Clientes
    ...
    # Botón para refrescar saldo manualmente debajo de la métrica
                ...
                if not f_desc_upper or not f_fecha.strip():
                    ...
                    if not df.empty:
                        for _, r in df.iterrows():
                            ...
                            if d_check and not d_check.startswith("---") and not d_check.startswith("TOTALES") and not d_check.startswith("El mes"):
...
if pestana_seleccionada == "Utilidad":
    ...
    try:
        ...
        try:
            sheet_utilidad.update(data_util)
        except Exception:
    ...
    if pestana_seleccionada == "Abono a tarjeta de crédito" and df.empty:
        # Si la tabla está vacía, forzar la creación de la cabecera mensual inicial con el saldo de clientes
        reorganized = reorganizar_con_meses_en_sheet(pd.DataFrame(), "Abono a tarjeta de crédito")
        headers = ["Mes", "Abono TC", "Descripcion", "Tarjeta Debito", "Saldo TC", "FECHA"]
        sheet_data = [headers]
        for r in reorganized:
            fila_limpia = [str(r.get(h, "")) if r.get(h, "") is not None else "" for h in headers]
    ...
    # Sincronización automática de la tarjeta al cargar cualquier pestaña si es Cliente
    if pestana_seleccionada == "Clientes":
        recalcular_y_sincronizar_tarjeta_tc()
    ...
    if not df.empty:
        for i, r in df.iterrows():
            if pestana_seleccionada == "Abono a tarjeta de crédito":
                identif = str(r.get("Descripcion", r.get("DESCRIPCION", ""))).strip()
            else:
                identif = str(r.get("FACTURA", "")).strip()
                
            if identif and not identif.startswith("---") and not identif.startswith("TOTALES") and not identif.startswith("El mes"):
                fec = str(r.get("FECHA", "")).strip()
                etiqueta = f"Fila {i+2} - {identif} ({fec})"
                opciones_dict[etiqueta] = i + 2
    ...
    if not opciones_dict:
        ...
        def actualizar_campos_edicion():
            ...
            if seleccion_actual != "-- Selecciona un registro --" and seleccion_actual in opciones_dict:
                try:
                    idx_sel = opciones_dict[seleccion_actual]
                    fila_df = df.iloc[idx_sel - 2]
        ...
        if seleccion == "-- Selecciona un registro --":
            ...
            idx_seleccionado = opciones_dict[seleccion]
            ...
            with st.form("form_editar_factura"):
                ...
                if eliminar:
                    ...
                    for i, r in df.iterrows():
                        if (i + 2) == idx_seleccionado:
                            continue 
                        val_check = str(r.get("FACTURA", r.get("Abono TC", r.get("ABONO TC", "")))).strip()
                        if val_check and not val_check.startswith("---") and not val_check.startswith("TOTALES") and not val_check.startswith("El mes"):
                            current_records.append(r.to_dict())
