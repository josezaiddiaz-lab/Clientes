...
def reorganizar_con_meses_en_sheet(df_actual):
    ...
    def agregar_totales_mes(m_num, regs_mes):
        ...
        if "IVA" in df_actual.columns:
            fila_tot["IVA"] = f"{t_iva:.3f}"
        if "ISR" in df_actual.columns:
            fila_tot["ISR"] = f"{t_isr:.3f}"
        if "TOTAL" in df_actual.columns:
            fila_tot["TOTAL"] = f"{t_tot:.3f}"
        return fila_tot
...
with st.sidebar.form("alta_form", clear_on_submit=True):
    ...
    if submitted:
        # Forzar mayúsculas en los textos ingresados
        factura_upper = factura.strip().upper()
        cliente_upper = cliente.strip().upper()
        descripcion_upper = descripcion.strip().upper()

        if factura_upper and cliente_upper and descripcion_upper and fecha and subtotal >= 0:
            ...
            if factura_upper in facturas_existentes:
                st.error(f"¡Error! El número de factura '{factura_upper}' ya existe en la base de datos para otro cliente.")
            else:
                try:
                ...
                new_row = {
                    "FACTURA": factura_upper,
                    "CLIENTES": cliente_upper,
                    "DESCRIPCION": descripcion_upper,
                    "FECHA": str(fecha),
    ...
    if df.empty:
        ...
        if seleccion == "-- Selecciona un registro --":
            ...
            with st.form("form_editar_factura"):
                ...
                e_iva_input = st.text_input("IVA (16% automático)", value=f"{calc_e_iva:.3f}")
                e_isr_input = st.text_input("ISR (1.25% automático)", value=f"{calc_e_isr:.3f}")
                
                st.info(f"Total estimado: ${calc_e_total:.3f}")
                ...
                if actualizar:
                    e_factura_upper = e_factura.strip().upper()
                    e_cliente_upper = e_cliente.strip().upper()
                    e_descripcion_upper = e_descripcion.strip().upper()
                    ...
                    if e_factura_upper in facturas_existentes:
                        st.error(f"¡Error! El número de factura '{e_factura_upper}' ya pertenece a otro registro.")
                    else:
                        try:
                        ...
                        updated_row = {
                            "FACTURA": e_factura_upper,
                            "CLIENTES": e_cliente_upper,
                            "DESCRIPCION": e_descripcion_upper,
                            "FECHA": e_fecha.strip(),
